  ...
  await pipelineInSequentialTasks(
    () => {
      for (const [segmentPath, segmentData] of segments) {
        const segmentChunks: SegmentCacheValue = {
          chunks: {
            [RenderStage.Static]: [],
            [RenderStage.Runtime]: [],
            [RenderStage.Dynamic]: [],
...
function createValidationCache(): SegmentCache {
  return new Map()
}

export type SegmentCache = Map<SegmentPath, SegmentCacheValue>

type SegmentCacheValue = {
  chunks: StageChunks
  debugChunks: Uint8Array[] | null
}
